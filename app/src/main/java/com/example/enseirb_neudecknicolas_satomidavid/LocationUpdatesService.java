...
public class LocationUpdatesService extends Service {
    ...
    static final String EXTRA_DISTANCE = PACKAGE_NAME + ".distance";
    ...
    private DatabankHandler db = null;
    ...
    private void endRun(){
        int current_runs = db.getCurrentRunCount();
        if(current_runs > 0){
            List<CurrentRun> currentRuns = db.getAllCurrentRun();
            Date dateStart = currentRuns.get(0).getTime();
            Date dateEnd = currentRuns.get(currentRuns.size() - 1).getTime();
            ...
            for(int i = 0; i < currentRuns.size(); i++){
                if(i > 0) json += ",";
                Location tmp = currentRuns.get(i).getLocation();
                json += "\"" + tmp.getLatitude() + "\":\"" + tmp.getLongitude() + "\"";
                if(i > 0){
            ...
            if(averageSpeedCounter > 0) averageSpeed = averageSpeed / averageSpeedCounter;
            db.addRun(new Run(user, dateStart, (int) distance, time, (int) averageSpeed, (int) topSpeed, locationList));
    ...
    private Notification getNotification() {
        ...
        double distance_nice = (distance_so_far / 1000);
        distance_nice -= distance_nice % 0.01;
        distance_nice *= 100d;
        distance_nice = ((int) distance_nice) / 100d;
        CharSequence content_text = "Distance so far: " + distance_nice + "m";

        CharSequence content_title = user.getFirst_name() + " is running at the moment!";
    ...
    private void onNewLocation(Location location) {
        ...
        if(mLocation != null && location != null){
            distance_so_far += mLocation.distanceTo(location);
        }
        ...
        intent.putExtra(EXTRA_DISTANCE, distance_so_far);
    ...
    public float getDistance_so_far() {
